...
const fs = require('fs').promises;
const path = require('path');
const DatabaseMigrations = require('./migrations'); // Optional: run your migration script
require('dotenv').config();
...
class DatabaseSetup {
  ...
  async createDatabase() {
    try {
      ...
      const dbName = process.env.DB_NAME || 'abe_garage';
  ...
  async importSchema() {
    try {
      ...
      console.log('🔄 Importing schema (this may take a moment)...');
      await connection.query(schemaSQL); // FIX: use query() for multi-statements
  ...
  async getTableInfo() {
    try {
      ...
      console.log('================================');
      tables.forEach(table => {
        console.log(`📋 ${table.TABLE_NAME} — Rows: ${table.TABLE_ROWS || 0} — Size: ${((table.DATA_LENGTH + table.INDEX_LENGTH) / 1024).toFixed(2)} KB`);
      });
  ...
  async runFullSetup() {
    ...
    // Optional: Run migrations for constraints, views, etc.
    try {
      console.log('\n🔄 Running migrations...');
      const migrations = new DatabaseMigrations();
      await migrations.runAllMigrations();
    } catch (e) {
      console.warn('⚠️ Migrations skipped or failed:', e.message);
    }
...
if (require.main === module) {
  const setup = new DatabaseSetup();
  setup.runFullSetup().then(success => process.exit(success ? 0 : 1));
}
